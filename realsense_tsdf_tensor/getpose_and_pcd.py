...
class PipelineModel:
    ...
    def __init__(self, camera_config_file=None, rgbd_video=None, device=None):
        ...
        if rgbd_video:  # 视频文件
            ...
            # 创建配置对象
            sensor_config = o3d.t.io.RealSenseSensorConfig()
    ...
    async def run(self):
        ...
        client = RobotClient("ws://192.168.10.1:1880")
        ...
        robot_rstates_real = []
        # 得到当前的位姿状态
        pose_and_joint_info = await client.get_geom()
        pose_and_joint = pose_and_joint_info.get("d")
        pose = pose_and_joint[0:6]
        joint = pose_and_joint[6:12]
        ...
        translation_data=45
        poses = np.array(
            [pose + np.array([0, 0, 0, angle_data / 2, 0, 0]), pose - np.array([0, 0, 0, angle_data / 2, 0, 0]),
             pose + np.array([0, 0, 0, angle_data, 0, 0]), pose - np.array([0, 0, 0, angle_data, 0, 0]),
             pose + np.array([0, 0, 0, 0, angle_data / 2, 0]), pose - np.array([0, 0, 0, 0, angle_data / 2, 0]),
             pose + np.array([0, 0, 0, 0, angle_data, 0]), pose - np.array([0, 0, 0, 0, angle_data, 0]),
             pose + np.array([0, 0, 0, 0, 0, angle_data / 2]), pose - np.array([0, 0, 0, 0, 0, angle_data / 2]),
             pose + np.array([0, 0, 0, 0, 0, angle_data]), pose - np.array([0, 0, 0, 0, 0, angle_data]),
             pose + np.array([0, 0, 0, angle_data / 2, angle_data / 2, 0]),
             pose - np.array([0, 0, 0, angle_data / 2, angle_data / 2, 0]),
             pose + np.array([0, 0, 0, -angle_data / 2, angle_data / 2, 0]),
             pose - np.array([0, 0, 0, -angle_data / 2, angle_data / 2, 0]),
             pose + np.array([0, 0, 0, angle_data / 2, 0, angle_data / 2]),
             pose - np.array([0, 0, 0, angle_data / 2, 0, angle_data / 2]),
             pose + np.array([0, 0, 0, -angle_data / 2, 0, angle_data / 2]),
             pose - np.array([0, 0, 0, -angle_data / 2, 0, angle_data / 2]),
             pose + np.array([0, 0, 0, 0, angle_data / 2, angle_data / 2]),
             pose - np.array([0, 0, 0, 0, angle_data / 2, angle_data / 2]),
             pose + np.array([0, 0, 0, 0, -angle_data / 2, angle_data / 2]),
             pose - np.array([0, 0, 0, 0, -angle_data / 2, angle_data / 2]),
             pose + np.array([0, translation_data / 2, 0, 0, 0, 0]),
             pose - np.array([0, translation_data / 2, 0, 0, 0, 0]),
             pose + np.array([0, translation_data, 0, 0, 0, 0]), pose - np.array([0, translation_data, 0, 0, 0, 0]),
             pose + np.array([0, 0, translation_data / 2, 0, 0, 0]),
             pose - np.array([0, 0, translation_data / 2, 0, 0, 0]),
             pose + np.array([0, 0, translation_data, 0, 0, 0]), pose - np.array([0, 0, translation_data, 0, 0, 0]),
             pose + np.array([translation_data / 2, 0, 0, 0, 0, 0]),
             pose - np.array([translation_data / 2, 0, 0, 0, 0, 0]),
             pose + np.array([translation_data, 0, 0, 0, 0, 0]), pose - np.array([translation_data, 0, 0, 0, 0, 0]),
             pose + np.array([translation_data / 2, translation_data / 2, 0, 0, 0, 0]),
             pose - np.array([translation_data / 2, translation_data / 2, 0, 0, 0, 0]),
             pose + np.array([-translation_data / 2, translation_data / 2, 0, 0, 0, 0]),
             pose - np.array([-translation_data / 2, translation_data / 2, 0, 0, 0, 0]),
             pose + np.array([translation_data / 2, 0, translation_data / 2, 0, 0, 0]),
             pose - np.array([translation_data / 2, 0, translation_data / 2, 0, 0, 0]),
             pose + np.array([-translation_data / 2, 0, translation_data / 2, 0, 0, 0]),
             pose - np.array([-translation_data / 2, 0, translation_data / 2, 0, 0, 0]),
             pose + np.array([0, translation_data / 2, translation_data / 2, 0, 0, 0]),
             pose - np.array([0, translation_data / 2, translation_data / 2, 0, 0, 0]),
             pose + np.array([0, -translation_data / 2, translation_data / 2, 0, 0, 0]),
             pose - np.array([0, -translation_data / 2, translation_data / 2, 0, 0, 0])
             ])
        ...
        for i in range(num_state):
            rtmp = R.as_matrix(R.from_euler('xyz', np.deg2rad(np.array(poses[i, 3:6]))))
            ttmp = poses[i, 0:3].T
            target_matrix = np.eye(4)
            target_matrix[0:3, 0:3] = rtmp
            target_matrix[0:3, 3] = ttmp / 1000
            robot_Rstates.append(rtmp)
            robot_tstates.append(np.array(ttmp / 1000))
            robot_rstates.append(np.deg2rad(np.array(poses[i, 3:6]).T))
            await client.move_line_and_wait(poses[i, 0:6])
            await asyncio.sleep(1)
            # 保存运动完成后的位姿
            pose_tmp_info_0 = await client.get_geom()
            ...
            while True:
                ...
                response = client.wait_for_send(response)
            ...
            # pose_tmp=pose_tmp_info.get("d")
            xyz1_tmp = np.array(pose_tmp[0:3])
            eu_tmp = np.deg2rad(np.array(pose_tmp[3:6]))
            rmatrix_tmp = R.as_matrix(R.from_euler('xyz', eu_tmp))
            target_matrix_real = np.eye(4)
            target_matrix_real[0:3, 0:3] = rmatrix_tmp
            xyz_tmp = xyz1_tmp.T
            target_matrix_real[0:3, 3] = xyz_tmp / 1000
            filename_T = str('./T_data/T_base2ee0' + str(i))
            np.savez_compressed(filename_T, arr_0=target_matrix_real)
            robot_Rstates_real.append(rmatrix_tmp)
            robot_rstates_real.append(np.array(eu_tmp).T)
            robot_tstates_real.append(np.array(xyz_tmp) / 1000)
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
            frame_id += 1
        await client.move_line_and_wait(pose)
        await client.close()
        self.executor.shutdown()
        print(robot_Rstates_real)
        print(robot_tstates_real)
        print(robot_rstates_real)
...
class PipelineController:
    ...
    def __init__(self, camera_config_file=None, rgbd_video=None, device=None):
        ...
        self.pipeline_model = PipelineModel(
                                            ...
                                            device)
        asyncio.run(self.pipeline_model.run())
