...
def main():
    ...
    for i in range(frame_count):
        # 加载和处理每一帧的图像和数据
        cam_pose_path = os.path.join(save_folder, f"pose_{i}.txt")  # 读取彩色图像
        cam_pose = np.loadtxt(cam_pose_path)  # 加载相机姿态文件
        cam_pose = np.eye(4)
        color_img_path = os.path.join(save_folder, f"color_{i}.png")  # 构建彩色图像文件路径
        color_image = cv2.imread(color_img_path, cv2.IMREAD_COLOR)  # 读取彩色图像

        depth_img_path = os.path.join(save_folder, f"depth_{i}.png")  # 构建深度图像文件路径
        depth_im = cv2.imread(depth_img_path, cv2.IMREAD_UNCHANGED).astype(float) / 1000  # 读取深度图像
    ...
    tsdf_vol = fusion.TSDFVolume(vol_bnds, voxel_size=0.0005)  # 创建TSDF体素体积对象，体素大小为2cm
    ...
    for i in tqdm(range(frame_count)):
        ...
        color_image = cv2.imread(color_img_path, cv2.IMREAD_COLOR)  # 重新读取彩色图像
        depth_image = cv2.imread(depth_img_path, cv2.IMREAD_UNCHANGED).astype(float) / 1000  # 重新读取深度图像并转换单位为米
        ...
        point_cloud = tsdf_vol.get_point_cloud()  # 从TSDF体积提取点云
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(point_cloud[:, :3])  # 设置点云坐标
        if point_cloud.shape[1] > 3:
            pcd.colors = o3d.utility.Vector3dVector(point_cloud[:, 3:] / 255.0)  # 设置点云颜色
        pcd_filename = os.path.join(pcd_folder, f"frame-{i:06d}.pcd")  # 构建点云文件名
        o3d.io.write_point_cloud(pcd_filename, pcd)  # 保存点云文件
